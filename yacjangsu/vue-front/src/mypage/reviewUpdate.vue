<template>
  <div class="con">
    ...
    <div class="container">
      ...
      <div class="myinfo">
        <p style="font-size:170%; margin: 0 60px 30px 250px;">리뷰 수정</p>
        <div class="write">
              ...
              <div class="goods_content">
                <div class="goodsbox">
                  ...
                  <div class="input-group">
                    <span class="input-group-text">{{ getMyReview.ORDERS_GOODS_NAME }}</span>
                  </div>
                </div>
                ...
                <div class="goodsbox">
                  ...
                  <img
                    :src="getMyReview.ORDERS_GOODS_IMG ? require(`../../../node-back/uploads/uploadGoods/${getMyReview.ORDERS_GOODS_IMG}`) : '/goodsempty.jpg'">
                </div>

                <div class="goodsbox">
                  <label class="form-label">카테고리</label>
                  <div class="input-group">
                    <span class="input-group-text">{{ categoryType(getMyReview.GOODS_CATEGORY) }}</span>
                  </div>
                </div>
                ...
                <div class="goodsbox">
                  ...
                  <div class="input-group">
                    <span class="input-group-text">{{ formatPrice(getMyReview.ORDERS_GOODS_PRICE) }}</span>
                  </div>
                </div>

                <div class="goodsbox">
                  <label class="form-label">상세 리뷰</label>
                  <textarea v-model="getMyReview.REVIEW_CON" class="form-control2" rows=10></textarea>
                </div>
                ...
                <div class="goodsbox" style="display: flex; align-items: center;">
                  ...
                  <img v-if="!getMyReview.REVIEW_IMG" src="../assets/imgEmptyInput.png" alt="..." width="50%;">
                  <img v-else :src="require(`../../../node-back/uploads/uploadReview/${getMyReview.REVIEW_IMG}`)" alt="..."
              width="50%;">
                </div>
                <!-- <div class="goodsbox" style="display: flex; align-items: center;">
  ...
  methods: {
    ...
    async handleWriteSubmit() {
      try {
        ...
        const response = await axios({
          method: 'post',
          url: 'http://localhost:3000/mypage/reviewEdit',
          data: {
            review_con: this.getMyReview.REVIEW_CON,
            //review_img: this.reviews.reviewImg,
            review_score: this.reviews.reviewScore,
            review_no: this.getMyReview.REVIEW_NO,
          },
          headers: { "Content-Type": "application/json" }
        });
        if (response.status === 200) {
          this.$swal('리뷰가 수정되었습니다.')
    ...
    async getReviewContent() {
      try {
        ...
        const response = await axios.get(`http://localhost:3000/mypage/getMyReview3/${reviewno}`)
        this.getMyReview = response.data[0]
        console.log(this.getReviewData, "ddd")
      } catch (error) {
        console.error(error);
      }
