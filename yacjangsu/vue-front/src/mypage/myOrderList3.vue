  ...
  export default {
    ...
    methods: {
      ...
      orderConfirm(ORDERS_NO) {
        ...
        for (const order of this.orderList) {
          const selectedOrdersWithStatus2 = this.order.filter(orderNos => {
          const order = this.orderList.find(order => order.ORDERS_NO === orderNos);
          return order && order.ORDERS_STATUS === 2;
        });
  
        if (selectedOrdersWithStatus2.length !== this.order.length) {
              ...
              if (result.value) {
                axios({
                  ...
                  axios.post(`http://localhost:3000/goods/orderPoint/${ORDERS_NO}/${this.user.user_no}`)
                    .then(() => {
                      window.location.href = `http://localhost:8080/mypage/myOrderList`;
                    })
                    .catch(error => {
                      console.error(error);
                    });
          ...
          else if(this.orderList.ORDERS_STATUS != 2){
